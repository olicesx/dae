...
1. Interface matcher is available in routing and DNS rules: `interface(wan:0eth)` or `interface(lan:3eth,4eth)`. `wan` only supports out semantic, and `lan` only supports in semantic.

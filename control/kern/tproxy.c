...
#define _l7proto_type ctx->params->flag[8]
...
lookup_lpm:
	...
	case MatchType_IpVersion:
		...
		if (_ipversion_type & match_set->ip_version)
			ctx->goodsubrule = true;
		break;
	case MatchType_L7Proto:
#ifdef __DEBUG_ROUTING
		bpf_printk(
			"CHECK: l7proto, match_set->type: %u, not: %d, outbound: %u",
			match_set->type, match_set->not, match_set->outbound);
#endif
		if (_l7proto_type & match_set->l7proto_type)
			ctx->goodsubrule = true;
		break;
...
new_connection:;
	...
	if (is_bittorrent(skb, offset) || is_utp(skb, l4proto, offset) || is_udp_tracker(skb, l4proto, offset)) {
		params.flag[8] = L7ProtoType_Bittorrent;
	} else {
		params.flag[8] = L7ProtoType_X;
	}

...
// DnsCorpusExpected describes the observable response shape. Zero values
// mean "no assertion on this field". Set Rcode to assert the response code
// (NOERROR, REFUSED, etc.), AnswerCount to assert a specific count, and
// AnswerIPv4 / AnswerIPv6 to assert a specific returned address.
...
// assertDnsShape checks only the asserted fields. Unset fields (zero Rcode,
// zero AnswerCount, empty AnswerIPv4 / AnswerIPv6) are skipped.
func assertDnsShape(t *testing.T, want DnsCorpusExpected, got *dnsmessage.Msg) {
	t.Helper()
	if want.Rcode != 0 && got.Rcode != want.Rcode {
		t.Fatalf("rcode = %d, want %d", got.Rcode, want.Rcode)
	}
	if want.AnswerCount != 0 && len(got.Answer) != want.AnswerCount {

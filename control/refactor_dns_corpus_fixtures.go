		...
		Cases: []DnsCorpusCase{
			{
				...
				PreState: func(t *testing.T, ctrl *DnsController) {
					...
					// Use a 60s TTL so the entry is fresh enough to be served;
					// the optimistic refresh path is exercised only when the
					// controller heuristics pick it up, but the externally
					// observable contract is the same: cached IP returned.
					if err := ctrl.UpdateDnsCacheTtlWithKey(

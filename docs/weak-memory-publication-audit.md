...
| High, safe | Routing policy in BPF maps | `PrepareRoutingEpoch`, routing builder, DNS cache replay, `StageRoutingEpoch`, `PublishRoutingEpoch` | `route()` in `control/kern/tproxy.c` | Rules, routing metadata, unique LPM references, and domain projection are populated in the inactive slot. The control plane retains the policy epoch alongside that slot. Only then does one `active_routing_epoch_map[0]` update expose the slot. `route()` samples the selector once and uses that slot for every keyed lookup. | New selector plus incomplete new-slot content is excluded by update order and inactive-slot isolation. This is a kernel protocol, not a Go atomic-publication claim. |

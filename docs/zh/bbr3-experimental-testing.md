...
| 实现性质 | 自制实现，**未验证 BBRv3 一致性，不是参考实现**（交付率估计已改为复用参考采样器） |
| hint 门控 | 经诊断证明惰性（13/13 run 零 lift），**默认关闭，不建议开启** |

结论：**bbr3 是"速率未知"时的默认控制器**（Hysteria2 / TUIC / Juicity）。
判据来自瓶颈链路实测（4 MB/s 限速 + 256 KB 队列 + 40 ms 单向，n=5）：速率未知时 bbr3
吞吐 3.31 MiB/s、p95 61.0 ms、丢包约 68，优于 BBR 的 2.99 MiB/s、63.8 ms、约 1200；
而**速率已知时不再使用 bbr3**——服务端协商 brutal 且链接声明了带宽（或 Hysteria2 服务端
广播了 Rx）时安装固定速率的 Brutal，实测 3.82 MiB/s、p95 19.9 ms，两根轴都更好。
测试目的仍是收集真实网络下的对照数据，不是宣称全面提升。
...
不设置 `cc_override` 时按上面的判据分流：速率未知走 bbr3，速率已知（服务端协商 brutal 且
链接声明了带宽，或 Hysteria2 服务端广播了 Rx）走固定速率的 Brutal。
